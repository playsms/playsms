...
find $PLAYSMS/storage/application/app/ -iname "*.php" -exec xgettext -L PHP --omit-header --no-location --sort-output --from-code=utf-8 -j -o $PLAYSMS/storage/tmp/application/plugin/language/messages.pot {} \;
find $PLAYSMS/storage/application/lib/ -iname "*.php" -exec xgettext -L PHP --omit-header --no-location --sort-output --from-code=utf-8 -j -o $PLAYSMS/storage/tmp/application/plugin/language/messages.pot {} \;
