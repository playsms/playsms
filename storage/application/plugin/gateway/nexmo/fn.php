	...
	if ($sms_sender && $sms_to && $sms_msg) {
		...
		if ($resp['message-count']) {
			...
			$db_query = "
				...
				VALUES (?,?,?,?,?)";
			if (dba_insert_id($db_query, [$smslog_id, $c_message_id, $c_status, $c_network, $c_error_text]) && $c_status === 0) {
				$ok = true;
				$p_status = 1;
...
function nexmo_hook_call($requests)
{
	...
	if ($access == 'callback') {
		$fn = $core_config['apps_path']['plug'] . '/gateway/nexmo/callback.php';
		_log("start load:" . $fn, 2, "nexmo call");
		include $fn;
		_log("end load callback", 2, "nexmo call");
	}
}
