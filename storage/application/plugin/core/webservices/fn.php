...
function webservices_ds($c_username, $queue_code = '', $src = '', $dst = '', $datetime = '', $smslog_id = 0, $c = 100, $last = false)
{
	...
	if ($uid) {
		...
		if ($j > 0) {
			...
			if (
				dba_search(_DB_PREF_ . '_tblSMSOutgoing_queue', 'id', array(
...
function webservices_output($operation, $requests, $returns)
{
	...
	if (
		$ret_intercept = core_hook('phonebook', 'webservices_output', array(
			$operation,
			$requests,
			$returns
		)
	...
	for ($c = 0; $c < count($core_config['plugins']['list']['feature']); $c++) {
		if (
			$ret_intercept = core_hook($core_config['plugins']['list']['feature'][$c], 'webservices_output', array(
				$operation,
				$requests,
				$returns
			)
	...
	if (is_array($gateways)) {
		...
		foreach ( $gateways as $gateway ) {
			if (
				$ret_intercept = core_hook($gateway, 'webservices_output', array(
					$operation,
					$requests,
					$returns
				)
	...
	// plugin themes
	if (
		$ret_intercept = core_hook(core_themes_get(), 'webservices_output', array(
			$operation,
			$requests,
			$returns
		)
...
function webservices_credit_view($username)
{
	...
	$json['status'] = 'OK';
	$json['error'] = '0';
	$json['balance'] = core_display_credit($credit);
...
function webservices_credit_add($username, $amount)
{
	...
	if (rate_addusercredit($uid, $amount)) {
		$json['status'] = 'OK';
		$json['error'] = '0';
		$json['amount'] = core_display_credit($amount);
...
function webservices_login_key_set($username)
{
	...
	if (
		registry_update($uid, 'core', 'webservices', array(
			'login_key' => $login_key
		)
		)
	) {
