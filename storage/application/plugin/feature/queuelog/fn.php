...
function queuelog_get($line_per_page = 0, $offset = 0)
{
	global $user_config;
	$ret = [];
	$db_argv = [];
	...
	if ((int) $line_per_page > 0) {
		$line_per_page_query = "LIMIT " . (int) $line_per_page;
	}
	if ((int) $offset > 0) {
		$limit_query = "OFFSET " . (int) $offset;
	}
	...
	while ($db_row = dba_fetch_array($db_result)) {
		$c_id = $db_row['id'];
		$db_row['count'] = dba_count(
			_DB_PREF_ . '_tblSMSOutgoing_queue_dst',
			array(
				'flag' => 0,
				'queue_id' => $c_id
			)
