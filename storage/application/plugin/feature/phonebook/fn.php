...
function phonebook_hook_phonebook_getdatabynumber($uid, $mobile)
{
	...
	if ($uid && core_mobile_matcher_format($mobile)) {
		...
		$db_query = "
			...
			LIMIT 1";
		$db_argv = [
			"%" . core_mobile_matcher_format($mobile),
			$uid,
			"%" . core_mobile_matcher_format($user_mobile),
			$uid
		];
...
{
	$ret = array();
	$db_query = "SELECT id AS gpid, name AS gp_name, code AS gp_code, flag_sender FROM " . _DB_PREF_ . "_featurePhonebook_group WHERE id=?";
	$db_result = dba_query($db_query, [$gpid]);
	if ($db_row = dba_fetch_array($db_result)) {
		$ret = $db_row;
	}
...
{
	$ret = array();
	$db_query = "SELECT id AS gpid, name AS gp_name, code AS gp_code, flag_sender FROM " . _DB_PREF_ . "_featurePhonebook_group WHERE uid=?";
	if ($orderby) {
		$db_query .= " ORDER BY " . $orderby;
	}
	$db_result = dba_query($db_query, [$uid]);
...
function phonebook_hook_phonebook_search($uid, $keyword = "", $count = 0, $exact = FALSE)
{
	...
	if ($keyword) {
		...
		$db_argv['uid'] = $uid;
		$db_argv['user_mobile'] = $user_mobile;
		if ((int) $count > 0) {
			$db_query .= " LIMIT " . (int) $count;
		}
		$db_result = dba_query($db_query, $db_argv);
		while ($db_row = dba_fetch_array($db_result)) {
			$ret[] = $db_row;
...
function phonebook_hook_phonebook_search_group($uid, $keyword = "", $count = 0, $exact = FALSE)
{
	...
	if ($keyword) {
		...
		$db_query = "
			...
			WHERE (
				...
				id in (
					...
					LEFT JOIN " . _DB_PREF_ . "_featurePhonebook_group_contacts AS C ON A.id=C.pid
					LEFT JOIN " . _DB_PREF_ . "_featurePhonebook_group AS B ON B.id=C.gpid
					WHERE A.mobile LIKE :user_mobile AND B.flag_sender=1
				) OR (
		...
		$db_argv['uid'] = $uid;
		$db_argv['user_mobile'] = "%" . core_mobile_matcher_format($user_mobile);
		$db_result = dba_query($db_query, $db_argv);
		while ($db_row = dba_fetch_array($db_result)) {
			$ret[] = $db_row;
		}
...
function phonebook_group_add($uid, $group_name, $group_code, $flag_sender = 0)
{
	...
	$db_result = dba_query($db_query, [$uid, $group_code]);
	if ($db_row = dba_fetch_array($db_result)) {
