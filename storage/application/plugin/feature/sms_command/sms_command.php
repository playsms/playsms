...
switch (_OP_) {
	case "sms_command_list":
		...
		$content .= "<tbody>";
		if (auth_isadmin()) {
			$db_query = "SELECT * FROM " . _DB_PREF_ . "_featureCommand WHERE uid=? ORDER BY command_keyword";
			$db_result = dba_query($db_query, [$user_config['uid']]);
		} else {
			$db_query = "SELECT * FROM " . _DB_PREF_ . "_featureCommand ORDER BY command_keyword";
		...
		while ($db_row = dba_fetch_array($db_result)) {
			if ($owner = user_uid2username($db_row['uid'])) {
				...
				$action .= _confirm(
					_('Are you sure you want to delete SMS command ?') . " (" . _('keyword') . ": " . $db_row['command_keyword'] . ")",
					_u('index.php?app=main&inc=feature_sms_command&op=sms_command_del&command_id=' . $db_row['command_id']),
					'delete'
				);
	...
	case "sms_command_edit_yes":
		...
		if ($command_id && $edit_command_keyword && $edit_command_exec) {
			...
			$edit_command_exec = str_replace("|", "", $edit_command_exec);
			$db_query = "
				UPDATE " . _DB_PREF_ . "_featureCommand 
				SET c_timestamp='" . time() . "',command_exec=?,command_return_as_reply=? WHERE command_keyword=?";
			if (dba_affected_rows($db_query, [$edit_command_exec, $edit_command_return_as_reply, $edit_command_keyword])) {
				...
				$_SESSION['dialog']['info'][] = _('No changes has been made') . " (" . _('keyword') . ": $edit_command_keyword)";
			}
		} else {
	...
	case "sms_command_add_yes":
		...
		if ($add_command_keyword && $add_command_exec) {
			...
			if (keyword_isavail($add_command_keyword)) {
				$db_query = "
					INSERT INTO " . _DB_PREF_ . "_featureCommand (uid,command_keyword,command_exec,command_return_as_reply,smsc) 
					VALUES (?,?,?,?,?)";
				$db_argv = [
					$user_config['uid'],
					$add_command_keyword,
					$add_command_exec,
					$add_command_return_as_reply,
					$add_smsc
				];
