...
function _credit_get_billing($uid)
{
	...
	if ($c_uid = (int) $uid) {
		$db_query = "SELECT SUM(charge) AS billing FROM " . _DB_PREF_ . "_tblBilling WHERE uid=? AND status=1";
		$db_result = dba_query($db_query, [$c_uid]);
		$db_row = dba_fetch_array($db_result);
...
function _credit_get_billing_parent($parent_uid)
{
	...
	if ($c_parent_uid = (int) $parent_uid) {
		$db_query = "SELECT SUM(charge) AS billing FROM " . _DB_PREF_ . "_tblBilling WHERE parent_uid=? AND status=1";
		$db_result = dba_query($db_query, [$c_parent_uid]);
		$db_row = dba_fetch_array($db_result);
		$billing = (float) $db_row['billing'];
	}
...
function _credit_update_credit($uid, $balance)
{
	...
	if ($c_uid = (int) $uid) {
		$db_query = "UPDATE " . _DB_PREF_ . "_tblUser SET c_timestamp='" . time() . "', credit=?, adhoc_credit=? WHERE uid=? AND flag_deleted=0";
		dba_affected_rows($db_query, [$balance, $balance, $c_uid]);
	}
}
