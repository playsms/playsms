...
Minimum required softwares:
  ...
  * Operating System Linux (install on MS Windows or BSD is possible, but currently not supported)
...
3.  Run installer script. Be warned that the script have been tested only on Ubuntu server or desktop.
    ...
    Note:
      ...
      * you must read and verify the install data presented when running script
        install-playsms-ubuntu.sh, and continue only when the data is correct
