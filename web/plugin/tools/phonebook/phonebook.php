...
switch (_OP_) {
	case "phonebook_list":
		...
		$base_url = 'index.php?app=main&inc=tools_phonebook&op=phonebook_list';
		...
		$content = "
			<h2>"._('Phonebook')."</h2>
			<p>".$search['form']."</p>
			<form name=fm_phonebook_list id=fm_phonebook_list action='index.php?app=main&inc=tools_phonebook' method=post>
			"._CSRF_FORM_."
			<input type=hidden id=action_route name=route value=''>
			<div class=actions_box>
				<div class=pull-left>
					<a href='"._u('index.php?app=main&inc=tools_phonebook&route=group&op=list')."'>".$icon_config['group']."</a>
					<a href='"._u('index.php?app=main&inc=tools_phonebook&route=import&op=list')."'>".$icon_config['import']."</a>
					<a href='"._u('index.php?app=main&inc=tools_phonebook&op=actions&go=export')."'>".$icon_config['export']."</a>
					<a href='"._u('index.php?app=main&inc=tools_phonebook&op=phonebook_add')."'>".$icon_config['add']."</a>
				</div>
		...
		for ($j=0;$j<count($list);$j++) {
			...
			for ($k=0;$k<count($grouplist);$k++) {
				$group_code .= $phonebook_flag_sender[$grouplist[$k]['flag_sender']]."<a href=\""._u('index.php?app=main&inc=tools_phonebook&route=group&op=edit&gpid='.$grouplist[$k]['id'])."\">".strtoupper($grouplist[$k]['code'])."</a><br />";
			}
			$i--;
			$c_i = "<a href=\""._u('index.php?app=main&inc=tools_phonebook&op=phonebook_edit&id='.$pid)."\">".$i.".</a>";
			$content .= "
				<tr>
					<td><a href='"._u('index.php?app=main&inc=tools_phonebook&op=phonebook_edit&pid='.$pid)."'>".$name."</a></td>
					<td>$mobile</td>
					<td>$email</td>
					<td>$group_code</td>
					<td>
	...
	case "phonebook_add":
		...
		$content = "
			<h2>"._('Phonebook')."</h2>
			<h3>"._('Add contact')."</h3>
			<form action=\"index.php?app=main&inc=tools_phonebook&op=actions&go=add\" name=fm_addphone method=POST>
			"._CSRF_FORM_."
	...
	case "phonebook_edit":
		...
		$content = "
			...
			<form action=\"index.php?app=main&inc=tools_phonebook&op=actions&go=edit\" name=fm_addphone method=POST>
			"._CSRF_FORM_."
