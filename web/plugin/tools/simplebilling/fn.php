...
function simplebilling_hook_billing_post($smslog_id,$rate,$credit,$count,$charge) {
	global $datetime_now;
	$ok = false;
	logger_print("saving smslog_id:".$smslog_id." rate:".$rate." credit:".$credit." count:".$count." charge:".$charge, 3, "simplebilling post");
	...
	if ($id = @dba_insert_id($db_query)) {
		logger_print("saved smslog_id:".$smslog_id." id:".$id, 3, "simplebilling post");
		$ok = true;
	}
	return $ok;
}
...
function simplebilling_hook_billing_rollback($smslog_id) {
	...
	if ($db_row = dba_fetch_array($db_result)) {
		$id = $db_row['id'];
		logger_print("saving smslog_id:".$smslog_id." id:".$id, 3, "simplebilling rollback");
		$db_query = "UPDATE "._DB_PREF_."_tblBilling SET status='2' WHERE id='$id'";
		if ($db_result = dba_affected_rows($db_query)) {
			logger_print("saved smslog_id:".$smslog_id, 3, "simplebilling rollback");
			$ok = true;
		}
	}
	return $ok;
...
function simplebilling_hook_billing_finalize($smslog_id) {
	...
	if ($db_result = dba_affected_rows($db_query)) {
		logger_print("saved smslog_id:".$smslog_id, 3, "simplebilling finalize");
		$ok = true;
	}
	return $ok;
}
