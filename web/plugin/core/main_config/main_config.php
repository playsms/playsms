...
switch (_OP_) {
	case "main_config":
		...
		$option_enable_register = _options([_('yes') => 1, _('no') => 0], $main_config['enable_register']);
		
		// enable forgot yes-no option
		$option_enable_forgot = _options([_('yes') => 1, _('no') => 0], $main_config['enable_forgot']);
		
		// disable login as subuser yes-no option
		$option_disable_login_as = _options([_('yes') => 1, _('no') => 0], $main_config['disable_login_as']);
		
		// enhance privacy for subusers
		$option_enhance_privacy_subuser = _options([_('yes') => 1, _('no') => 0], $main_config['enhance_privacy_subuser']);
		
		// enable logo yes-no option
		$option_enable_logo = _options([_('yes') => 1, _('no') => 0], $main_config['enable_logo']);
		
		// enable logo to replace main website title yes-no option
		$option_logo_replace_title = _options([_('yes') => 1, _('no') => 0], $main_config['logo_replace_title']);
		
		// option default account status on user registration
		$option_default_user_status = _options([_('User') => 3, _('Subuser') => 4], $main_config['default_user_status']);
	...
	case "main_config_save":
		...
		$logo_url = trim((string) $_POST['edit_logo_url']);
		...
		foreach ($_POST as $key => $val) {
			if (str_starts_with($key, 'edit_')) {
				$post[$key] = str_replace('"', '\'', (string) $val);
			}
		}
		
		$items = ['web_title' => $post['edit_web_title'], 'email_service' => $post['edit_email_service'], 'email_footer' => $post['edit_email_footer'], 'main_website_name' => $post['edit_main_website_name'], 'main_website_url' => $post['edit_main_website_url'], 'gateway_number' => core_sanitize_sender($post['edit_gateway_number']), 'gateway_timezone' => $post['edit_gateway_timezone'], 'username_length' => ((((int) $post['edit_username_length'] >= 3) && ((int) $post['edit_username_length'] <= 100)) ? (int) $post['edit_username_length'] : 30), 'default_rate' => (float) $post['edit_default_rate'], 'gateway_module' => ($post['edit_gateway_module'] ?: 'dev'), 'themes_module' => ($post['edit_themes_module'] ?: 'default'), 'language_module' => ($post['edit_language_module'] ?: 'en_US'), 'sms_max_count' => (int) ($post['edit_sms_max_count'] > 1 ? $post['edit_sms_max_count'] : 1), 'plus_sign_remove' => (int) $post['edit_plus_sign_remove'], 'plus_sign_add' => (int) $post['edit_plus_sign_add'], 'enable_credit_unicode' => (int) $post['edit_enable_credit_unicode'], 'brute_force_detection' => (int) $post['edit_brute_force_detection'], 'keyword_separator' => substr((string) $post['edit_keyword_separator'], 0, 1), 'credit_lowest_limit' => (float) $post['edit_credit_lowest_limit'], 'sms_limit_per_hour' => (int) $post['edit_sms_limit_per_hour'], 'default_replace_zero' => $post['edit_default_replace_zero'], 'default_credit' => (float) $post['edit_default_credit'], 'default_user_status' => $edit_default_user_status, 'default_parent' => (int) $post['edit_default_parent'], 'default_acl' => (int) $post['edit_default_acl'], 'enable_register' => (int) $post['edit_enable_register'], 'enable_forgot' => (int) $post['edit_enable_forgot'], 'disable_login_as' => (int) $post['edit_disable_login_as'], 'enhance_privacy_subuser' => (int) $post['edit_enhance_privacy_subuser'], 'enable_logo' => (int) $enable_logo, 'logo_url' => $logo_url, 'logo_replace_title' => (int) $logo_replace_title, 'layout_footer' => ($post['edit_layout_footer'] ?: _('Application footer here. Go to main configuration or manage site to edit this footer.')), 'information_title' => ($post['edit_information_title'] ?: _('Information')), 'information_content' => ($post['edit_information_content'] ?: _('Go to main configuration or manage site to edit this page'))];
