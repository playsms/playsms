...
function sendmail($data = array()) {
	...
	if (is_array($data)) {
		...
		_log("error no data", 3, "sendmail");

		return false;
	}
	
	_log("start from:" . $mail_from . " to:" . $mail_to . " subject:" . $mail_subject, 3, "sendmail");
	...
	try {
		...
		_log("sent from:" . $mail_from . " to:" . $mail_to . " subject:" . $mail_subject, 3, "sendmail");

		return true;
	} catch (Exception $e) {
		_log("end with error:" . $mail->ErrorInfo, 3, "sendmail");
