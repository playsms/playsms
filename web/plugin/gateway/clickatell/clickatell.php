...
if ($clickatell_param['ready'])
{
	$status_active = "(<b><font color=green>"._('Ready')."</font></b>) (<a href=\"index.php?app=menu&inc=gateway_clickatell&op=manage_unready\">"._('click here to set gateway unready')."</a>)";
}
else
{
	$status_active = "(<b><font color=red>"._('Unready')."</font></b>) (<a href=\"index.php?app=menu&inc=gateway_clickatell&op=manage_ready\">"._('click here to set gateway ready')."</a>)";
}
...
switch ($op)
{
	...
	<table width=100% cellpadding=1 cellspacing=2 border=0>
	    ...
	    <tr>
		<td>"._('Module sender ID')."</td><td>:</td><td><input type=text size=16 maxlength=16 name=up_sender value=\"".$clickatell_param['global_sender']."\"> ("._('Max. 16 numeric or 11 alphanumeric char. empty to disable').")</td>
	    </tr>	    
	    <tr>
	...
	case "manage_save":
		...
		break;
	case "manage_ready":
		$db_query = "UPDATE "._DB_PREF_."_gatewayClickatell_config SET ready=TRUE";
		...
		break;
	case "manage_unready":
		$db_query = "UPDATE "._DB_PREF_."_gatewayClickatell_config SET ready=FALSE";
		$db_result = dba_query($db_query);
