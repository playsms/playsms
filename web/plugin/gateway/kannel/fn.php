...
function kannel_hook_sendsms($smsc, $sms_sender, $sms_footer, $sms_to, $sms_msg, $uid = '', $gpid = 0, $smslog_id = 0, $sms_type = 'text', $unicode = 0) {
	...
	$URL = "/cgi-bin/sendsms?username=" . urlencode((string) $plugin_config['kannel']['username']) . "&password=" . urlencode(htmlspecialchars_decode((string) $plugin_config['kannel']['password']));
	$URL .= "&from=" . urlencode((string) $sms_sender) . "&to=" . urlencode($sms_to);
	...
	if ($additional_param = htmlspecialchars_decode((string) $plugin_config['kannel']['additional_param'])) {
