...
define('_CALLBACK_GATEWAY_LOG_MARKER_', _CALLBACK_GATEWAY_NAME_ . ' geturl');
...
if ($remote_addr != $plugin_config['kannel']['bearerbox_host'] && $remote_host != $plugin_config['kannel']['bearerbox_host']) {
	_log("unable to process incoming SMS. remote_addr:[" . $remote_addr . "] or remote_host:[" . $remote_host . "] does not match with your bearerbox_host config:[" . $plugin_config['kannel']['bearerbox_host'] . "] smsc:[" . $smsc . "]", 2, _CALLBACK_GATEWAY_LOG_MARKER_);
	exit();
}
...
_log("remote_addr:" . $remote_addr . " remote_host:" . $remote_host . " t:[" . $t . "] q:[" . $q . "] a:[" . $a . "] Q:[" . $Q . "] smsc:[" . $smsc . "] smsc:[" . $smsc . "]", 3, _CALLBACK_GATEWAY_LOG_MARKER_);
...
if ($t && $q && $a) {
	...
	_log("missing parameter t:[" . $t . "] q:[" . $q . "] a:[" . $a . "] smsc:[" . $smsc . "]", 2, _CALLBACK_GATEWAY_LOG_MARKER_);
}
