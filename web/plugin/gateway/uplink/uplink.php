...
if ($uplink_param['ready'])
{
	$status_active = "(<b><font color=green>"._('Ready')."</font></b>) (<a href=\"index.php?app=menu&inc=gateway_uplink&op=manage_unready\">"._('click here to set gateway unready')."</a>)";
}
else
{
	$status_active = "(<b><font color=red>"._('Unready')."</font></b>) (<a href=\"index.php?app=menu&inc=gateway_uplink&op=manage_ready\">"._('click here to set gateway ready')."</a>)";
}
...
switch ($op)
{
	...
	case "manage_save":
		...
		break;
	case "manage_ready":
		$db_query = "UPDATE "._DB_PREF_."_gatewayUplink_config  SET ready=TRUE";
		...
		break;
	case "manage_unready":
		$db_query = "UPDATE "._DB_PREF_."_gatewayUplink_config  SET ready=FALSE";
		$db_result = dba_query($db_query);
