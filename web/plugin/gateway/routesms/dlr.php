...
if (!$called_from_hook_call) {
    ...
    chdir("plugin/gateway/routesms/");
    $requests = $_REQUEST;
}

if (!$requests) $requests = $_POST;
...
_log("dlr request: " . $xml, 3, "routesms dlr");

//preg_match_all('/id=\"([0-9]+)\"/', $xml, $result);
//$apimsgid = $result[1][0];
$apimsgid = $requests['sMessageId'];
_log("apimsgid: " . $apimsgid, 3, "routesms dlr");

//if (preg_match_all('/status=\"([A-Z]+)\"/', $xml, $result)) {
//	$status = $result[1][0];
//}
$status = $requests['sStatus'];

$db_query = "SELECT smslog_id FROM " . _DB_PREF_ . "_gatewayRoutesms_apidata WHERE apimsgid='$apimsgid'";
...
_log("addr:" . $remote_addr . " host:" . $remote_host . " type:" . $status . " smslog_id:" . $smslog_id . " uid:" . $uid, 2, "routesms dlr");
...
if ($status && $smslog_id && $uid) {
	...
	// if ($db_result > 0) {
	$db_query = "UPDATE " . _DB_PREF_ . "_gatewayRoutesms_apidata SET c_timestamp='" . time() . "', status='$status' WHERE smslog_id='$smslog_id'";
	$db_result = dba_query($db_query);
        
        $db_query = "SELECT p_status FROM " . _DB_PREF_ . "_tblSMSOutgoing WHERE smslog_id='$smslog_id'";
        ...
        if ($pstatus == 0 && $status != 0):
            ...
            $db_query = "UPDATE " . _DB_PREF_ . "_tblSMSOutgoing SET p_credit='$charge', c_timestamp='" . time() . "', p_status='$status', p_update='$datedone' WHERE smslog_id='$smslog_id'";
            $dbb_result = dba_query($db_query);
        endif;
