...
if (!$core_config['daemon_process']) {
	$callback_url = $_SERVER['HTTP_HOST'] . dirname((string) $_SERVER['PHP_SELF']) . "/plugin/gateway/generic/callback.php";
...
if (!trim((string) $plugin_config['generic']['url'])) {
	$plugin_config['generic']['url'] = $plugin_config['generic']['default_url'];
}
if (!trim((string) $plugin_config['generic']['callback_url'])) {
	$plugin_config['generic']['callback_url'] = $plugin_config['generic']['default_callback_url'];
}
if (!trim((string) $plugin_config['generic']['callback_url_authcode'])) {
	$plugin_config['generic']['callback_url_authcode'] = sha1((string) _PID_);
}

// smsc configuration
$plugin_config['generic']['_smsc_config_'] = ['url' => _('Generic send SMS URL'), 'api_username' => _('API username'), 'api_password' => _('API password'), 'module_sender' => _('Module sender ID'), 'datetime_timezone' => _('Module timezone')];
