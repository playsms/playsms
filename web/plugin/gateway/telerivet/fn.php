...
function telerivet_hook_sendsms($smsc, $sms_sender, $sms_footer, $sms_to, $sms_msg, $uid = '', $gpid = 0, $smslog_id = 0, $sms_type = 'text', $unicode = 0) {
	...
	# Catch query error
	if ($network_error) { 
		_log('curl error:' . $network_error,2,'telerivet query');
		dlr($smslog_id, $uid, 2);
	} else {
		# Save JSON reply
		...
		if (is_array($res)) {
			...
			_log('api response:' . $log,3,'telerivet query');
		}
		# Catch API errors
		if (isset($res['error'])) {
			_log('api error' . $res['error_message'],2,'telerivet query');
		} else {
			_log('api success: id:' . $res['id'] . ' status:' . $res['status'] . ' source:' . $res['source'],2,'telerivet query');
		}
