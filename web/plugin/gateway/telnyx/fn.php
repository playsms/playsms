...
function telnyx_hook_sendsms($smsc, $sms_sender, $sms_footer, $sms_to, $sms_msg, $uid = '', $gpid = 0, $smslog_id = 0, $sms_type = 'text', $unicode = 0) {
	...
	_log("enter smsc:" . $smsc . " smslog_id:" . $smslog_id . " uid:" . $uid . " to:" . $sms_to, 2, "telnyx_hook_sendsms");
	...
	_log("sendsms start", 3, "telnyx_hook_sendsms");
	...
	if ($sms_sender && $sms_to && $sms_msg) {
		...
		if (trim($plugin_config['telnyx']['callback_url'])) {
			$data['delivery_status_webhook_url'] = trim($plugin_config['telnyx']['callback_url']);
		}
		...
		if (function_exists('curl_init')) {
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    			...
    			'x-profile-secret: ' . $plugin_config['telnyx']['auth_token']
			));
			...
			curl_close($ch);
			_log("sendsms url:[" . $url . "] callback:[" . $plugin_config['telnyx']['callback_url'], "] smsc:[" . $smsc . "]", 3, "telnyx_hook_sendsms");
			$resp = json_decode($returns);
			if ($resp->status) {
				$c_status = $resp->status;
				$c_message_id = $resp->sms_id;
				$c_error_text = $c_status;
				_log("sent smslog_id:" . $smslog_id . " message_id:" . $c_message_id . " status:" . $c_status . " error:" . $c_error_text . " smsc:[" . $smsc . "]", 2, "telnyx_hook_sendsms");
				$db_query = "
					INSERT INTO " . _DB_PREF_ . "_gatewayTelnyx (local_smslog_id,remote_smslog_id,status,error_text)
				...
				_log("failed smslog_id:" . $smslog_id . " resp:" . $resp . " smsc:[" . $smsc . "]", 2, "telnyx_hook_sendsms");
			}
		} else {
