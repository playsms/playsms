...
switch (_OP_) {
	...
	case "manage_save":
		...
		if ($up_url && $up_api_username) {
			$items = ['url' => $up_url, 'callback_url' => $up_callback_url, 'api_username' => $up_api_username, 'module_sender' => $up_module_sender, 'datetime_timezone' => $up_datetime_timezone];
			if ($up_api_password) {
