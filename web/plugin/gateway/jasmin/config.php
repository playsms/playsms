...
if (!$core_config['daemon_process']) {
	$callback_url = $_SERVER['HTTP_HOST'] . dirname((string) $_SERVER['PHP_SELF']) . "/plugin/gateway/jasmin/callback.php";
...
if (!trim((string) $plugin_config['jasmin']['url'])) {
	$plugin_config['jasmin']['url'] = $plugin_config['jasmin']['default_url'];
}
if (!trim((string) $plugin_config['jasmin']['callback_url'])) {
	$plugin_config['jasmin']['callback_url'] = $plugin_config['jasmin']['default_callback_url'];
}

// smsc configuration
$plugin_config['jasmin']['_smsc_config_'] = ['url' => _('Jasmin send SMS URL'), 'callback_url' => _('Callback URL'), 'api_username' => _('API username'), 'api_password' => _('API password'), 'module_sender' => _('Module sender ID'), 'datetime_timezone' => _('Module timezone')];
