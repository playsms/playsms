...
function common_hook_themes_apply($content) {
	...
	$themes_lang = strtolower(substr((string) $user_config['language_module'], 0, 2));
	
	if ($themes_layout = trim((string) $_REQUEST['_themes_layout_'])) {
		$themes_layout = 'themes_layout_' . $themes_layout;
	} else {
		$themes_layout = 'themes_layout';
	}
	
	$tpl = ['name' => $themes_layout, 'vars' => ['CONTENT' => $content, 'HTTP_PATH_BASE' => $core_config['http_path']['base'], 'HTTP_PATH_THEMES' => $core_config['http_path']['themes'], 'THEMES_MODULE' => core_themes_get(), 'THEMES_MENU_TREE' => themes_get_menu_tree(), 'THEMES_SUBMENU' => themes_submenu(), 'THEMES_LANG' => ($themes_lang ?: 'en'), 'CREDIT_SHOW_URL' => _u('index.php?app=ws&op=credit'), 'NAME' => $user_config['name'], 'USERNAME' => $user_config['username'], 'GRAVATAR' => $user_config['opt']['gravatar'], 'LAYOUT_FOOTER' => $core_config['main']['layout_footer'], 'Logout' => _('Logout')], 'ifs' => ['valid' => auth_isvalid()]];
	$content = tpl_apply($tpl, ['core_config', 'user_config']);
...
function common_hook_themes_buildmenu($menu_config) {
	...
	foreach ($menu_config as $menu_title => $array_menu) {
		foreach ($array_menu as $sub_menu) {
			...
			$sub_menu_index = (int) ($sub_menu[2] ?: 10) + 100;
