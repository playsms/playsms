...
switch ($op)
{
	...
	<table width=100% cellpadding=1 cellspacing=2 border=0>
	    <tr>
		<td width=150>SMS board keyword</td><td width=5>:</td><td><b>$edit_board_keyword</b></td>
	    </tr>
	    <tr>
		<td>Forward to email</td><td>:</td><td><input type=text size=30 name=edit_email value=\"$edit_email\"></td>
	    </tr>	    
	    <tr>
		<td colspan=3>
		Template:
	...
	if ($edit_board_id)
	{
	    ...
	    {
	        $error_string = "SMS board `$edit_board_keyword` has been saved";
	    }
	}
	else
	{
	...
	if ($board_keyword)
	{
	    ...
	    if (@dba_affected_rows($db_query))
	    {
		$error_string = "SMS board `$board_keyword` with all its messages has been deleted!";
	    }
	}
	...
	<table width=100% cellpadding=1 cellspacing=2 border=0>
	    <tr>
		<td width=150>SMS board keyword</td><td width=5>:</td><td><input type=text size=30 maxlength=30 name=add_board_keyword value=\"$add_board_keyword\"></td>
	    </tr>
	    <tr>
		<td colspan=3><p><b>Leave them empty if you dont know what to fill in these boxes below</b></td>
	    </tr>	    
	    <tr>
		<td>Forward to email</td><td>:</td><td><input type=text size=30 name=add_email value=\"$add_email\"></td>
	    </tr>	    	    
	    <tr>
		<td colspan=3>
		Template:
	...
	if ($add_board_keyword)
	{
		...
		{
		    $error_string = "SMS board `$add_board_keyword` has been added";
		}
		else
		{
		    $error_string = "Fail to add SMS board `$add_board_keyword`";
		}
	    }
	    else
	    {
		$error_string = "SMS keyword `$add_board_keyword` already exists, reserved or use by other feature!";
	    }
	}
	else
	{
