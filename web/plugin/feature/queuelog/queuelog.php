...
switch (_OP_) {
	case "queuelog_list":
		...
		for ($c = count($data) - 1; $c >= 0; $c--) {
			...
			$c_message = $data[$c]['message'];
			$c_action = "<a href=\"javascript: ConfirmURL('" . _("Are you sure you want to delete queue") . " " . $c_queue_code . " ?','" . _u('index.php?app=main&inc=feature_queuelog&op=queuelog_delete&queue=' . $c_queue_code) . "')\">" . $icon_config['delete'] . "</a>";
			$content .= "
				<tr>
			";
			if (auth_isadmin()) {
