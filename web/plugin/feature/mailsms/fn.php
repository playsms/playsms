...
function mailsms_hook_playsmsd() {
	...
	foreach ($users as $user ) {
		...
		// get mailsms registry data for $uid
		$items = registry_search($uid, 'features', 'mailsms');
		
		$enable = $items['features']['mailsms']['enable'];
		$protocol = $items['features']['mailsms']['protocol'];
		$port = $items['features']['mailsms']['port'];
		$server = $items['features']['mailsms']['server'];
		$username = $items['features']['mailsms']['username'];
		$password = $items['features']['mailsms']['password'];
...
function mailsms_hook_playsmsd_once($param) {
	...
	if ($uid && $username) {
		...
		$ssl = ($items['features']['mailsms']['ssl'] == 1) ? "/ssl" : "";
		$novalidate_cert = ($items['features']['mailsms']['novalidate_cert'] == 1) ? "/novalidate-cert" : "";
		$email_hostname = '{' . $items['features']['mailsms']['server'] . ':' . $items['features']['mailsms']['port'] . '/' . $items['features']['mailsms']['protocol'] . $ssl . $novalidate_cert . '}INBOX';
		$email_username = $items['features']['mailsms']['username'];
		$email_password = $items['features']['mailsms']['password'];
		...
		if (count($emails)) {
			...
			foreach ($emails as $email_number ) {
				...
				if ($items['features']['mailsms']['check_sender']) {
				...
				$message = trim(preg_replace('/' . $items['features']['mailsms']['pin'] . '/', '', $email_subject, -1, $count));
				if ($count <= 0) {
					_log('PIN does not match. Subject: ' . $email_subject, 2, 'mailsms_hook_playsmsd');
				}
				
				// sendsms
				if ($username && count($sms_to) && $message && $count > 0) {
					_log('mailsms username:' . $username, 3, 'mailsms_hook_playsmsd_once');
