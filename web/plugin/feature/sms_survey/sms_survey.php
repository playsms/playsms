...
if (($route = $_REQUEST['route']) && ($route == 'xls')) {
	include $core_config['apps_path']['plug'].'/feature/sms_survey/reportxls.php';
	exit();
}
...
switch ($op) {
	...
	//	echo $av;
		...
		$content .= "
			...
			<form method='post' action='index.php?app=menu&inc=feature_sms_survey&op=report'>
			<p><input class='button' type='submit' value='"._('View Report')."'></p>
			</form>
		";
		echo $content;
		break;
	case 'add':
		...
		break;
	case 'report':
		$strSQL = "SELECT * FROM playsms_featureSurvey ORDER by keyword";
		$rs = mysql_query($strSQL);
		$nr = mysql_num_rows($rs);
		if(isset($_POST['pilih'])) {
		$keyword = $_POST['pilih'];
		echo "Answer Of : <b>" . $keyword . "<b>";
		echo "<a href=/playsms/plugin/feature/sms_survey/reportxls.php?keyword=$keyword><br /> Export to Excel</a> ";
		$data = sms_survey_option_check($keyword);
			$content = "
				<table width='100%' cellpadding='1' cellspacing='2' border='0' class='sortable'>
                        <tr>
                                <td class='box_title' width='4'>*</td>
                                <td class='box_title' width='10%'>"._('Keyword')."</td>
                                <td class='box_title' width='20%'>"._('Sender')."</td>
                                <td class='box_title' width='50%'>"._('Message')."</td>
                                <td class='box_title' width='20%'>"._('Time')."</td>
                        </tr>
			...
			$content .= "
                                <tr class='".$td_class."'>
                                        <td align='center'>".($i+1).".</td>
                                        <td align='center'>".$data[$i]['keyword']."</td>
                                        <td align='center'>".$data[$i]['in_sender']."</td>
                                        <td align='center'>".$data[$i]['in_message']."</td>
                                        <td align='center'>".$data[$i]['in_datetime']."</td>
				</tr>	
			";
			}
		...
		$content = '<h2>'._('SMS Survey').'</h2><p />';
		$content .= '<h3>'._('Please Select Report Survey').'</h3><p />';
		$content .= "<form method='post' action='index.php?app=menu&inc=feature_sms_survey&op=report'>";
		$content .= '<SELECT name="pilih">';
                for ($i=0; $i<$nr; $i++) {
                        $r = mysql_fetch_array($rs);
                        $content .= '<OPTION VALUE="'.$r["keyword"].'">'.$r["keyword"].'</OPTION>';
                }
		$content .= "<br />";

		$content .= "<p><input class='button' type='submit' value='"._('See Report')."'></p>";
