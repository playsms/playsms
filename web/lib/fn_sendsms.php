...
function sendsms_get_sender($username) {
	...
	if ($username && ($gw = gateway_get())) {
		// 1st priority is "SMS sender ID" from user preferences
		if($core_config['user']['sender']){
		...
		elseif ($core_config['main']['cfg_gateway_number']) {
			// 2nd priority is "Default sender ID" from main configuration
			$sms_sender = $core_config['main']['cfg_gateway_number'];
		} else {
			// 3rd priority is "Module sender ID" from gateway module setting
