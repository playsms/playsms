...
function core_array_addslashes($array) {
	if (is_array($array)) {
		foreach ($array as $key => $value) {
			if (!is_array($value)) {
				$new_arr[$key] = addslashes((string) $value);
			}
			if (is_array($value)) {
 ...
 * @return mixed
 */
function core_addslashes(mixed $data) {
	if (is_array($data)) {
...
function core_array_htmlspecialchars($array) {
	if (is_array($array)) {
		foreach ($array as $key => $value) {
			if (!is_array($value)) {
				$new_arr[$key] = htmlspecialchars((string) $value);
			}
			if (is_array($value)) {
 ...
 * @return mixed
 */
function core_htmlspecialchars(mixed $data) {
	if (is_array($data)) {
 ...
 * @return mixed
 */
function core_sanitize_inputs(mixed $data) {
...
function core_setuserlang($username = "") {
	...
	if (trim((string) $db_row['language_module'])) {
...
function core_sanitize_filename($string) {
	$string = trim(preg_replace('/[^\p{L}\p{N}\s._-]+/u', '', (string) $string));
	
	return $string;
}


function core_hook($c_plugin, $c_function, $c_param = []) {
 ...
 * @return mixed
 */
function core_display_html(mixed $data) {
	$config = HTMLPurifier_Config::createDefault();
	$config->set('Attr.EnableID', TRUE);
	$config->set('HTML.SafeObject', TRUE);
	$config->set('HTML.SafeEmbed', TRUE);
	...
	if (is_array($data)) {
		...
		$value = htmlspecialchars_decode((string) $data); // here its started with $data, not $value
		$value = str_ireplace('<?php', '', $value);
		$value = str_ireplace('<?', '', $value);
		$value = str_ireplace('?>', '', $value);
		$value = str_ireplace('`', '', $value);
...
function core_get_timezone($username = '') {
	...
	$ret = '';
	if ($username) {
		$list = dba_search(_DB_PREF_ . '_tblUser', 'datetime_timezone', ['flag_deleted' => 0, 'username' => $username]);
...
function core_get_random_string($length = 16, $valid_chars = '') {
	$valid_chars = str_replace(' ', '', (string) $valid_chars);
	if (!$valid_chars) {
	...
	$result = "";
	for ($i = 0; $i < $length; $i++) {
		$index = random_int(0, $valid_char_len - 1);
...
function core_sanitize_username($username) {
	$username = preg_replace("/[^a-z\d._-]/i", '', (string) $username);
	return $username;
}
...
function core_sanitize_alphanumeric($string) {
	...
	$string = trim(preg_replace('/[^\p{L}\p{N}]+/u', '', (string) $string));
...
function core_sanitize_alpha($string) {
	...
	$string = trim(preg_replace('/[^\p{L}]+/u', '', (string) $string));
...
function core_sanitize_numeric($string) {
	...
	$string = trim(preg_replace('/[^\p{N}]+/u', '', (string) $string));
...
function core_xml_to_array($xml) {
	$loaded = simplexml_load_string((string) $xml);
	$json = json_encode($loaded, JSON_THROW_ON_ERROR);
	$var = json_decode($json, TRUE, 512, JSON_THROW_ON_ERROR);
	return $var;
}
...
function core_lang_get() {
	...
	$ret = core_call_hook();
	if (!$ret) {
		return ($user_config['language_module'] ?: $core_config['main']['language_module']);
	}
...
function core_mobile_matcher_format($mobile) {
	...
	if (strlen((string) $c_mobile) >= 6) {
		// remove +
		$c_mobile = str_replace('+', '', (string) $c_mobile);
