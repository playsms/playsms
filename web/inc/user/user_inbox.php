...
switch ($op) {
	case "user_inbox":
		...
		$db_query = "SELECT * FROM "._DB_PREF_."_tblUserInbox WHERE in_uid='$uid' AND in_hidden='0' ".$search_sql." ORDER BY in_id DESC LIMIT ".$nav['limit']." OFFSET ".$nav['offset'];
		$db_result = dba_query($db_query);
		$i = $nav['top'];
		$j = 0;
	...
	case "user_inbox_del":
		...
		if ($in_id = $_REQUEST['inid']) {
			$db_query = "UPDATE "._DB_PREF_."_tblUserInbox SET c_timestamp='".mktime()."',in_hidden='1' WHERE in_uid='$uid' AND in_id='$in_id'";
			$db_result = dba_affected_rows($db_query);
			if ($db_result > 0) {
				$_SESSION['error_string'] = _('Selected incoming SMS has been deleted');
			}
		}
		header("Location: index.php?app=menu&inc=user_inbox&op=user_inbox");
		exit();
		break;
	case "act_del":
		...
		for ($i=1;$i<=$item_count;$i++) {
			...
			$inid = $_POST['inid'.$i];
			if(($chkid=="on") && $inid) {
				$db_query = "UPDATE "._DB_PREF_."_tblUserInbox SET c_timestamp='".mktime()."',in_hidden='1' WHERE in_uid='$uid' AND in_id='$inid'";
				$db_result = dba_affected_rows($db_query);
			}
		}
		$_SESSION['error_string'] = _('Selected incoming SMS has been deleted');
